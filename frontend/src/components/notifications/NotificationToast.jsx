...
export default function NotificationToast({ notification, onDismiss, duration = 5000 }) {
  ...
  return (
    <div
      className={clsx(
        'flex items-start gap-3 p-4 bg-white dark:bg-surface-800 rounded-lg shadow-lg border border-surface-200 dark:border-surface-700',
        'transform transition-all duration-300 max-w-sm',
        isExiting ? 'opacity-0 translate-x-full' : 'opacity-100 translate-x-0'
      )}
    >
      <div className={clsx('p-2 rounded-lg text-white', colors[notification.type] || 'bg-surface-500')}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-surface-500 dark:text-surface-400 text-xs mt-1 line-clamp-2">
          {notification.body}
        </p>
      </div>
