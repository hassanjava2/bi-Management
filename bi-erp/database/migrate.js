/**
 * BI ERP - Migration runner (Laravel-style)
 * Run: node database/migrate.js (from bi-erp root)
 */
...
async function run() {
  const { initDatabase, getDatabase, run: dbRun, all } = require('../backend/src/config/database');
  ...
  let batch = (await all('SELECT COALESCE(MAX(batch), 0) as b FROM migrations'))[0]?.b || 0;
