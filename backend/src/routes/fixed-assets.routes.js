...
router.post('/', async (req, res) => {
    try {
        ...
        const id = generateId();
        run(`INSERT INTO fixed_assets (id, code, name, category, purchase_invoice_id, cost, is_expense_tracked) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [id, code || null, name, category || null, purchase_invoice_id || null, parseFloat(cost || 0), is_expense_tracked ? 1 : 0]);
        res.status(201).json({ success: true, data: get('SELECT * FROM fixed_assets WHERE id = ?', [id]) });
    } catch (e) {
...
router.get('/expense-types', async (req, res) => {
    try {
        const rows = all('SELECT * FROM fixed_asset_expense_types ORDER BY name');
        res.json({ success: true, data: rows });
    } catch (e) {
        if (e.message && e.message.includes('no such table')) return res.json({ success: true, data: [] });
        res.status(500).json({ success: false, error: e.message });
...
router.get('/:id/expenses', async (req, res) => {
    try {
        const rows = all('SELECT * FROM fixed_asset_expenses WHERE asset_id = ? ORDER BY expense_date DESC', [req.params.id]);
        res.json({ success: true, data: rows });
    } catch (e) {
        if (e.message && e.message.includes('no such table')) return res.json({ success: true, data: [] });
        res.status(500).json({ success: false, error: e.message });
...
router.post('/:id/expenses', async (req, res) => {
    try {
        ...
        const asset = get('SELECT id FROM fixed_assets WHERE id = ?', [req.params.id]);
        if (!asset) return res.status(404).json({ success: false, error: 'NOT_FOUND' });
        const id = generateId();
        run(`INSERT INTO fixed_asset_expenses (id, asset_id, type_id, amount, expense_date, notes) VALUES (?, ?, ?, ?, ?, ?)`,
            [id, req.params.id, type_id || null, parseFloat(amount), expense_date || now().slice(0, 10), notes || null]);
        res.status(201).json({ success: true, data: get('SELECT * FROM fixed_asset_expenses WHERE id = ?', [id]) });
    } catch (e) {
...
router.patch('/:id/assign', async (req, res) => {
    try {
        ...
        const at = now();
        run(`UPDATE fixed_assets SET assigned_employee_id = ?, assigned_at = ?, custody_status = 'assigned' WHERE id = ?`, [employee_id || null, at, req.params.id]);
        res.json({ success: true, data: get('SELECT * FROM fixed_assets WHERE id = ?', [req.params.id]) });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
});
