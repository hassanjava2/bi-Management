...
router.get('/', async (req, res) => {
    try {
        const rows = all('SELECT * FROM companies WHERE 1=1 ORDER BY name');
...
router.post('/', async (req, res) => {
    try {
        ...
        const id = generateId();
        run(`INSERT INTO companies (id, code, name, name_ar) VALUES (?, ?, ?, ?)`,
            [id, code || null, name, name_ar || null]);
        res.status(201).json({ success: true, data: get('SELECT * FROM companies WHERE id = ?', [id]) });
    } catch (e) {
