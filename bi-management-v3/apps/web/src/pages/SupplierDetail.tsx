import { useEffect, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { Row, Col, Card, Descriptions, Button, Tag, Space, Statistic, Alert } from "antd";
import {
  EditOutlined,
  PrinterOutlined,
  FileTextOutlined,
  WalletOutlined,
  PhoneOutlined,
  MailOutlined,
  EnvironmentOutlined,
  GlobalOutlined,
  UserOutlined,
} from "@ant-design/icons";
import { PageHeader, MoneyDisplay, DateDisplay, LoadingSkeleton } from "../components/shared";
import { API_BASE, getAuthHeaders, onAuthFailure } from "../utils/api";

type SupplierDetailType = {
  id: string;
  code: string | null;
  name: string;
  nameAr: string | null;
  type: string | null;
  contactPerson: string | null;
  phone: string | null;
  phone2: string | null;
  email: string | null;
  website: string | null;
  address: string | null;
  city: string | null;
  country: string | null;
  balance: number | null;
  creditLimit: number | null;
  isActive: number | null;
  notes: string | null;
};

export default function SupplierDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [data, setData] = useState<SupplierDetailType | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    document.title = "تفاصيل المورد | BI Management v3";
  }, []);

  useEffect(() => {
    if (!id) {
      setError("معرف المورد مطلوب");
      setLoading(false);
      return;
    }
    setLoading(true);
    setError("");
    fetch(`${API_BASE}/api/suppliers/${id}`, { headers: getAuthHeaders() })
      .then((res) => {
        if (res.status === 401) {
          onAuthFailure();
          throw new Error("انتهت الجلسة");
        }
        if (!res.ok) throw new Error("المورد غير موجود");
        return res.json();
      })
      .then(setData)
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  }, [id]);

  if (loading) {
    return (
      <div>
        <PageHeader
          title="تفاصيل المورد"
          breadcrumbs={[
            { title: "الموردين", href: "/suppliers" },
            { title: "تفاصيل المورد" },
          ]}
        />
        <LoadingSkeleton type="form" rows={6} />
      </div>
    );
  }

  if (error) {
    return (
      <div>
        <PageHeader
          title="تفاصيل المورد"
          breadcrumbs={[
            { title: "الموردين", href: "/suppliers" },
            { title: "خطأ" },
          ]}
        />
        <Alert
          message="خطأ"
          description={error}
          type="error"
          showIcon
          action={
            <Button type="link" onClick={() => navigate("/suppliers")}>
              العودة للموردين
            </Button>
          }
        />
      </div>
    );
  }

  if (!data) return null;

  const balanceLabel = (data.balance || 0) <= 0 ? "(لنا)" : "(علينا)";
  const balanceColor = (data.balance || 0) <= 0 ? "#3f8600" : "#cf1322";

  return (
    <div>
      <PageHeader
        title={data.nameAr || data.name}
        subtitle={data.nameAr ? data.name : undefined}
        breadcrumbs={[
          { title: "الموردين", href: "/suppliers" },
          { title: data.nameAr || data.name },
        ]}
        extra={
          <>
            <Button icon={<EditOutlined />} onClick={() => navigate(`/suppliers/${id}/edit`)}>
              تعديل
            </Button>
            <Button icon={<PrinterOutlined />} onClick={() => window.print()}>
              طباعة
            </Button>
          </>
        }
      />

      {/* Header Card with Status */}
      <Card style={{ marginBottom: 24 }}>
        <Row justify="space-between" align="middle">
          <Col>
            <Space size="middle">
              {data.type && <Tag color="purple">{data.type}</Tag>}
              <Tag color={data.isActive ? "green" : "red"}>
                {data.isActive ? "نشط" : "غير نشط"}
              </Tag>
              {data.code && (
                <Tag color="default" style={{ fontFamily: "monospace" }}>
                  كود: {data.code}
                </Tag>
              )}
            </Space>
          </Col>
        </Row>
      </Card>

      {/* Financial Summary */}
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} md={8}>
          <Card>
            <Statistic
              title="الرصيد"
              valueStyle={{ color: balanceColor, fontWeight: 600 }}
              formatter={() => (
                <Space>
                  <MoneyDisplay amount={Math.abs(data.balance || 0)} size="large" />
                  <span style={{ fontSize: 14, color: balanceColor }}>{balanceLabel}</span>
                </Space>
              )}
            />
          </Card>
        </Col>
        {data.creditLimit && (
          <Col xs={24} sm={12} md={8}>
            <Card>
              <Statistic
                title="حد الائتمان"
                valueStyle={{ color: "#1890ff", fontWeight: 600 }}
                formatter={() => <MoneyDisplay amount={data.creditLimit!} size="large" />}
              />
            </Card>
          </Col>
        )}
      </Row>

      {/* Contact Information */}
      <Card title="معلومات الاتصال" style={{ marginBottom: 24 }}>
        <Descriptions column={{ xs: 1, sm: 2, md: 3 }} bordered>
          {data.contactPerson && (
            <Descriptions.Item label={<><UserOutlined /> جهة الاتصال</>}>
              {data.contactPerson}
            </Descriptions.Item>
          )}
          {data.phone && (
            <Descriptions.Item label={<><PhoneOutlined /> الهاتف</>}>
              {data.phone}
            </Descriptions.Item>
          )}
          {data.phone2 && (
            <Descriptions.Item label={<><PhoneOutlined /> هاتف إضافي</>}>
              {data.phone2}
            </Descriptions.Item>
          )}
          {data.email && (
            <Descriptions.Item label={<><MailOutlined /> البريد الإلكتروني</>}>
              <a href={`mailto:${data.email}`}>{data.email}</a>
            </Descriptions.Item>
          )}
          {data.website && (
            <Descriptions.Item label={<><GlobalOutlined /> الموقع الإلكتروني</>}>
              <a href={data.website} target="_blank" rel="noreferrer">
                {data.website}
              </a>
            </Descriptions.Item>
          )}
        </Descriptions>
      </Card>

      {/* Address Information */}
      {(data.address || data.city || data.country) && (
        <Card title="العنوان" style={{ marginBottom: 24 }}>
          <Descriptions column={{ xs: 1, sm: 2, md: 3 }} bordered>
            {data.city && (
              <Descriptions.Item label={<><EnvironmentOutlined /> المدينة</>}>
                {data.city}
              </Descriptions.Item>
            )}
            {data.country && (
              <Descriptions.Item label={<><GlobalOutlined /> الدولة</>}>
                {data.country}
              </Descriptions.Item>
            )}
            {data.address && (
              <Descriptions.Item label={<><EnvironmentOutlined /> العنوان التفصيلي</>} span={2}>
                {data.address}
              </Descriptions.Item>
            )}
          </Descriptions>
        </Card>
      )}

      {/* Notes */}
      {data.notes && (
        <Card
          title="ملاحظات"
          style={{ marginBottom: 24, background: "#fffbe6", borderColor: "#ffe58f" }}
        >
          <p style={{ margin: 0, color: "#8c6d1f" }}>{data.notes}</p>
        </Card>
      )}

      {/* Quick Actions */}
      <Card title="إجراءات سريعة">
        <Space wrap>
          <Button
            type="primary"
            icon={<FileTextOutlined />}
            style={{ background: "#14b8a6" }}
            onClick={() => navigate(`/invoices/new?supplierId=${id}&type=purchases`)}
          >
            إنشاء فاتورة مشتريات
          </Button>
          <Button
            type="primary"
            icon={<WalletOutlined />}
            style={{ background: "#f97316" }}
            onClick={() => navigate(`/vouchers/new?supplierId=${id}&type=payment`)}
          >
            إنشاء سند صرف
          </Button>
          <Button
            icon={<EditOutlined />}
            onClick={() => navigate(`/suppliers/${id}/edit`)}
          >
            تعديل البيانات
          </Button>
        </Space>
      </Card>
    </div>
  );
}
