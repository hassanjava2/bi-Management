...
function ensureReturnsTable() {
  try {
    get('SELECT 1 FROM returns LIMIT 1');
    return true;
  } catch {
    return false;
  }
...
function nextReturnNumber() {
  ...
  const r = get('SELECT return_number FROM returns WHERE return_number LIKE ? ORDER BY created_at DESC LIMIT 1', [`RTN-${year}-%`]);
  let seq = 1;
...
function getById(id) {
  const r = get('SELECT r.*, s.name as supplier_name FROM returns r LEFT JOIN suppliers s ON r.supplier_id = s.id WHERE r.id = ?', [String(id)]);
  if (!r) return null;
  let items = [];
  try {
    items = all('SELECT * FROM return_items WHERE return_id = ?', [String(id)]);
  } catch (_) {}
  return { ...r, items };
}
...
function updateStatus(id, status, notes) {
  const existing = get('SELECT id FROM returns WHERE id = ?', [String(id)]);
  if (!existing) return null;
  run(`UPDATE returns SET status = ?, notes = COALESCE(notes, '') || ? WHERE id = ?`, [status, notes ? '\n' + notes : '', String(id)]);
  return getById(id);
}

function addFollowUp(id, action, notesVal) {
  const existing = get('SELECT id, notes FROM returns WHERE id = ?', [String(id)]);
  if (!existing) return null;
  const followUpText = `[${now()}] ${action}: ${notesVal || ''}`;
  run(`UPDATE returns SET notes = COALESCE(notes, '') || ? WHERE id = ?`, [followUpText + '\n', String(id)]);
  return getById(id);
}
...
function receive(id, data) {
  const existing = get('SELECT id FROM returns WHERE id = ?', [String(id)]);
  if (!existing) return null;
  run(`UPDATE returns SET status = 'completed', destination_warehouse_id = ?, processed_at = ?, notes = COALESCE(notes, '') || ? WHERE id = ?`, [data.warehouse_id || null, now(), data.notes ? '\nاستلام: ' + data.notes : '', String(id)]);
  return getById(id);
}

function getStats() {
  if (!ensureReturnsTable()) return null;
  const total = get('SELECT COUNT(*) as total FROM returns');
  const byStatus = all('SELECT status, COUNT(*) as cnt FROM returns GROUP BY status');
  const bySupplier = all('SELECT supplier_id, COUNT(*) as cnt FROM returns WHERE supplier_id IS NOT NULL GROUP BY supplier_id');
  const by_status = {};
...
function getOverdue() {
  const rows = all(`SELECT r.*, s.name as supplier_name FROM returns r LEFT JOIN suppliers s ON r.supplier_id = s.id WHERE r.status NOT IN ('completed', 'rejected', 'returned') ORDER BY r.created_at ASC`);
