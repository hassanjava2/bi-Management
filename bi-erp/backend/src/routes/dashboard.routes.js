...
router.get('/stats', async (req, res) => {
  try {
    ...
    const [totalCustomers, totalSuppliers, totalProducts, totalInvoices, todaySales, totalEmployees] = await Promise.all([
      ...
      get("SELECT COUNT(*) as c, COALESCE(SUM(total),0) as total FROM invoices WHERE type LIKE 'sale%' AND created_at::date = CURRENT_DATE AND (is_deleted = FALSE OR is_deleted IS NULL)").then(r => ({ count: r?.c || 0, total: r?.total || 0 })).catch(() => ({ count: 0, total: 0 })),
