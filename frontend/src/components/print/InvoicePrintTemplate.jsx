/**
 * BI Management - Invoice Print Templates
 * قوالب طباعة الفواتير (A4 + حراري)
 */
      ...
      <div className="flex justify-end">
        <div className="w-72">
          <div className="flex justify-between py-1 text-sm">
            ...
            <span>{formatNumber(inv.subtotal)} د.ع</span>
          </div>
          ...
          {parseFloat(inv.paid_amount) > 0 && parseFloat(inv.remaining_amount) > 0 && (
            <>
...
export const InvoiceThermal = forwardRef(function InvoiceThermal({ invoice, items, company }, ref) {
  ...
  return (
    <div ref={ref} className="print-only bg-white text-black mx-auto" dir="rtl" style={{ width: '80mm', fontFamily: 'Arial, Tahoma, sans-serif', fontSize: '12px', padding: '8px' }}>
      ...
      {/* Footer */}
      <div className="text-center mt-4 text-xs text-gray-400 border-t border-dashed border-gray-400 pt-3">
...
${isA4 ? `
  <div style="display:flex;justify-content:space-between;border-bottom:2px solid #000;padding-bottom:12px;margin-bottom:20px">
    <div><h1 style="margin:0;font-size:22px">${co.name}</h1><p style="margin:4px 0 0;color:#666;font-size:13px">${co.address || ''}</p><p style="margin:2px 0 0;color:#666;font-size:13px">${co.phone || ''}</p></div>
    <div style="text-align:left"><h2 style="margin:0;font-size:18px;color:#444">${typeLabel}</h2><p style="margin:4px 0 0;font-size:13px"><span style="color:#888">رقم: </span><strong style="font-family:monospace">${inv.invoice_number || ''}</strong></p><p style="margin:2px 0 0;font-size:13px"><span style="color:#888">التاريخ: </span>${formatDate(inv.created_at)}</p></div>
  </div>
  ...
  <div style="display:flex;justify-content:flex-end"><div style="width:250px">
    <div style="display:flex;justify-content:space-between;padding:3px 0;font-size:13px"><span style="color:#666">المجموع الفرعي:</span><span>${formatNumber(inv.subtotal)} د.ع</span></div>
...
` : `
  <div style="text-align:center;border-bottom:1px dashed #999;padding-bottom:8px;margin-bottom:8px"><h1 style="margin:0;font-size:16px">${co.name}</h1><p style="margin:2px 0 0;font-size:10px;color:#666">${co.phone || ''}</p><p style="margin:4px 0 0;font-size:12px;font-weight:bold">${typeLabel}</p></div>
  <div style="font-size:11px;margin-bottom:8px"><div style="display:flex;justify-content:space-between"><span>رقم:</span><span style="font-family:monospace;font-weight:bold">${inv.invoice_number || ''}</span></div><div style="display:flex;justify-content:space-between"><span>التاريخ:</span><span>${formatDate(inv.created_at)}</span></div>${inv.customer_name ? `<div style="display:flex;justify-content:space-between"><span>العميل:</span><span>${inv.customer_name}</span></div>` : ''}${inv.supplier_name ? `<div style="display:flex;justify-content:space-between"><span>المورد:</span><span>${inv.supplier_name}</span></div>` : ''}</div>
  ...
  <div style="text-align:center;margin-top:12px;font-size:10px;color:#aaa;border-top:1px dashed #999;padding-top:8px"><p style="margin:0">شكراً لتعاملكم معنا</p></div>
`}
</body></html>`
